...
namespace HardwareShopManagement
{
    public partial class POListForm : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                if(MessageBox.Show("Do you want to remove selected Purchase Order?","Confirm Remove",MessageBoxButtons.YesNo,MessageBoxIcon.Error)==System.Windows.Forms.DialogResult.Yes)
                {
                    string no = dg1.CurrentRow.Cells[0].Value.ToString();
                    Dictionary<string, object> dic = new Dictionary<string, object>();
                    dic.Add("@no", no);
                    db.Execute("delete from PO where PONo=@no", dic);
                    db.FillGrid(dg1, "select * from PO");
        ...
        private void btnedit_Click_1(object sender, EventArgs e)
        {
            if (dg1.CurrentRow != null)
            {
                string pono = dg1.CurrentRow.Cells[0].Value.ToString();
                POForm f = new POForm(pono);
                f.ShowDialog();
                db.FillGrid(dg1, "select * from PO");
            }
            else
        ...
        {
            db.FillGrid(dg1, "Select * from PO where " + cmbfield.Text + " like'%" + txtsearch.Text + "%'");
        }

        private void btnprint_Click(object sender, EventArgs e)
        {
            db.ShowReport(new POReport(), "select * from PO", "PO");
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                String pono=dg1.CurrentRow.Cells[0].Value.ToString();
                db.ShowReport (new PO2Report(),"select * from  POView where PoNO="+pono,"POView");
            }
        }

        private void txtsearch_TextChanged_1(object sender, EventArgs e)
        {
            db.FillGrid(dg1, "select * from POItems where " + cmbfield.Text + " like '%" + txtsearch.Text + "%'");
        }
